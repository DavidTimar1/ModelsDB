...
- `curated.json` - the human-readable OBJECTIVE export, written into the DB's data dir (git-ignored). It is re-exported whenever the app saves or runs `update`. Personal fields (`notes`, `favorite`) and any model flagged `unlisted` are NEVER exported - they live only in `modelsdb.db` (the source of truth), same as every custom column you define (see "Custom columns" below). Back up personal data by copying the DB.
...
- Curated, always preserved across a refresh, but split by whether they are exported:
  - *Objective* (exported to `curated.json`): `tool`, `moe`, `parameters`, `active_parameters`, `disk_size_gb`, `measurement`, `pricing_note`.
  - *Personal* (never exported - lives only in the DB): `notes`, `favorite`.
- **Custom columns** (personal, never exported): user-defined columns - a text field, or a single-select dropdown of text or number labels - created and deleted from the UI's **Custom columns** button, not from this skill. They live in their own tables (`custom_columns`, `custom_column_options`, `custom_values`), not as columns on `models`, so they are outside the scope of the direct-SQL workflow this skill describes. A fresh install already has three: **Speed** (fast/avg/slow), **Rating** (1-4), **OCR** (good/bad).
